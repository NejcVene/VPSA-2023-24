...
__device__ unsigned char vrniPiksel(unsigned char *imgIn, int row, int col, int channel, int width, int height, int cpp) {
...
__global__ void poudariRob(unsigned char *imgIn, unsigned char *imgOut) {

    int grow = blockIdx.y * blockDim.y + threadIdx.y;
    int gcol = blockIdx.x * blockDim.x + threadIdx.x;
    // izračunamo novo vrednost piksla tako, da delaš konvolucijo z masko

    if (grow < height && gcol < width) {
        for (size_t channel = 0; channel < cpp; channel++) {
            imgOut[row * width + col * cpp + channel] = 5 * vrniPiksel(imgIn, grow, gcol, channel, width, height, cpp);
        }
    }
...
int main(int argc, char *argv[]){
    ...
    // naredi prostor za sliko v pomnilniku (to zapiše v tale char kazalec)
