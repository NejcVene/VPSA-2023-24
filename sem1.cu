...
int main(int argc, char **argv){
    ...
    if (!(imageOut = (unsigned char *) malloc(height * width * sizeof(unsigned char)))) {
        err("malloc", "Could not allocate memory for output image on host")
    }

    // allocate memory on GPU and set it to 0
    // image
    checkCudaErrors(cudaMalloc(&imageInDevice, height * width * sizeof(unsigned char)));
    // histogram
    ...
    // minimum
    checkCudaErrors(cudaMalloc(&minimumDevice, sizeof(unsigned int)));
    // cudaMemset(minimumDevice, 0, 1 * sizeof(unsigned int));
    // imageOut
    checkCudaErrors(cudaMalloc(&imageOutDevice, height * width * sizeof(unsigned char)));
    checkCudaErrors(cudaMemset(imageOutDevice, 0, height * width * sizeof(unsigned char)));
